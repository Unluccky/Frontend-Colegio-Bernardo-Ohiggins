...
import { Calculator, TrendingUp, BookOpen, ClipboardList, Download } from 'lucide-react';
...
export default function MisNotasPage() {
  ...
  const promedioGeneral = grouped.length > 0
    ? Number((grouped.reduce((s, g) => s + g.promedio, 0) / grouped.length).toFixed(1))
    : 0;

  const handleDownloadPDF = async () => {
    if (!notasRef.current || misNotas.length === 0) return;
    setDownloading(true);
    try {
      const canvas = await html2canvas(notasRef.current, {
        scale: 2,
        useCORS: true,
      ...
      const nombre = studentEstudiante
        ? `${studentEstudiante.nombre}_${studentEstudiante.apellido}_Notas`.replace(/\s+/g, '_')
        : 'Mis_Notas';
      pdf.save(`${nombre}.pdf`);
    } catch (e) {
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      link.download = studentEstudiante
        ? `${studentEstudiante.nombre}_${studentEstudiante.apellido}_Notas.png`.replace(/\s+/g, '_')
        : 'Mis_Notas.png';
